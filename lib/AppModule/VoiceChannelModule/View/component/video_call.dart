...
import '../../Model/voice_channel_model.dart';
import 'call_action_row.dart';
...
class _VideoCallPageState extends State<VideoCallPage> {
  ...
  late final _users = <VoiceChannelModel>{};
  ...
  void _addAgoraEventHandlers() => _agoraEngine.setEventHandler(
        RtcEngineEventHandler(
          ...
          firstLocalAudioFrame: (elapsed) {
            ...
            debugPrint(info);
            for (VoiceChannelModel user in _users) {
          ...
          userOffline: (uid, elapsed) {
            ...
            debugPrint(info);
            VoiceChannelModel? userToRemove;
            for (VoiceChannelModel user in _users) {
              if (user.uid == uid) {
                userToRemove = user;
              }
...
class AgoraVideoLayout extends StatelessWidget {
  const AgoraVideoLayout({
    required Set<VoiceChannelModel> users,
  ...
  final Set<VoiceChannelModel> _users;
...
class AgoraVideoView extends StatelessWidget {
  const AgoraVideoView({
    required double viewAspectRatio,
    required VoiceChannelModel user,
  })  : _viewAspectRatio = viewAspectRatio,
        _user = user;

  final double _viewAspectRatio;
  final VoiceChannelModel _user;
